...
fun InitScreen(
    ...
    BoxWithConstraints(
        ...
        // maxWidth를 기반으로 이미지 크기 계산 (예: 화면 너비의 50%)
        val imageSize = maxWidth * 0.5f

        Column(
            modifier = Modifier.fillMaxWidth(), // Column은 너비만 꽉 채우도록 설정
            ...
            Image(
                ...
                modifier = Modifier.size(imageSize) // 계산된 크기 적용
            )
...
fun InitScreenPreview(modifier: Modifier = Modifier) {
    PhogalTheme {
        BoxWithConstraints(
            ...
            // 1. 화면 너비에 따라 이미지 크기 결정 (예: 너비의 40% 또는 최대 200.dp)
            val imageSize = maxWidth * 0.4f

            Column(
                // 2. 내부 Column에는 외부 modifier 대신 fillMaxSize() 계열 사용
                ...
                Image(
                    ...
                    modifier = Modifier.size(imageSize) // 3. 계산된 크기 적용 (Scope 사용)
                )
                Spacer(modifier = Modifier.height(12.dp))
                Text(
